...
public class generate_Parenthesis {
    ...
    public void process(int i, int left, int right, int n) {
        if (left > right)
            return;
        if (left == 0 && right == 0) {
            res.add(s);
        ...
        if (left > 0) {
            s = s + '(';
            process(i + 1, left - 1, right, n);
            s = s.length() == 1 ? "" : s.substring(0, s.length() - 1);
        }
        if (right > 0) {
            s = s + ')';
            process(i + 1, left, right - 1, n);
            s = s.length() == 1 ? "" : s.substring(0, s.length() - 1);
        }
    }
    //process()和process2()都可以,process简洁一点，process2更好理解一点
    public void process2(int i ,int left,int right,int n ){
